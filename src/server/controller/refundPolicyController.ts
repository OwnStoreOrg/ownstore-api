...
const register = async (server: Hapi.Server): Promise<void> => {
  server.realm.modifiers.route.prefix = '/refund-policy'
  ...
  registerCacheRoute(server, {
    ...
    handler: async (request: Hapi.Request): Promise<IStaticPageDetail> => {
      ...
      const tncDetail = await staticPageService.getRefundPolicyDetail()
  ...
  server.route({
    ...
    handler: async (request: Hapi.Request): Promise<IStaticPageUpdate> => {
      ...
      const updateRefundPolicy = await staticPageService.updateRefundPolicyDetail({
        title: payload.title,
        body: payload.body,
      })
