...
const register = async (server: Hapi.Server): Promise<void> => {
  server.realm.modifiers.route.prefix = '/shipping-policy'
  ...
  registerCacheRoute(server, {
    ...
    handler: async (request: Hapi.Request): Promise<IStaticPageDetail> => {
      ...
      const tncDetail = await staticPageService.getShippingPolicyDetail()
  ...
  server.route({
    ...
    handler: async (request: Hapi.Request): Promise<IStaticPageUpdate> => {
      ...
      const updateShippingPolicy = await staticPageService.updateShippingPolicyDetail({
        title: payload.title,
        body: payload.body,
      })

      return updateShippingPolicy
    },
  })
}
